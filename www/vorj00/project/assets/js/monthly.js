	...
	$.fn.extend({
		monthly: function(options) {
			...
			var options = $.extend(defaults, options),
				...
				monthNames = ["Leden", "Únor", "Březen", "Duben", "Květen", "Červen", "Červenen", "Srpen", "Září", "Říjen", "Listopad", "Prosinec"],
				dayNames = ['Pondělí','Úterý','Středa','Čtvrtek','Pátek','Sobota','Něděle'];
		...
		if (options.weekStart == 'Sun') {
			...
			$('#' + uniqueId).append('<div class="monthly-day-title-wrap"><div>Pon</div><div>Úte</div><div>Stř</div><div>Čtv</div><div>Pát</div><div>Sob</div><div>Ned</div></div><div class="monthly-day-wrap"></div>');
		} else {
		...
		function setMonthly(m, y){
			...
			if (setMonth == currentMonth && setYear == currentYear) {
				...
				$('#' + uniqueId + ' .monthly-header-title').html(monthNames[m - 1] +' '+ y +'<a href="#" class="monthly-reset" title="Zpátky k aktuálnímu měsíci"></a> ');
			}
			...
			if (options.mode == 'event') {
				...
				$.get(''+options.xmlUrl+'', function(d){
					$(d).find('event').each(function(){
						...
						var fullstartDate = $(this).find('event_startdate').text(),
							startArr = fullstartDate.split("-"),
							startYear = startArr[0],
							startMonth = parseInt(startArr[1], 10),
							startDay = parseInt(startArr[2], 10),
							fullendDate = $(this).find('event_enddate').text(),
							endArr = fullendDate.split("-"),
							endYear = endArr[0],
							endMonth = parseInt(endArr[1], 10),
							endDay = parseInt(endArr[2], 10),
							eventTitle = $(this).find('event_name').text(),
							eventColor = $(this).find('color').text() ? $(this).find('color').text() : '#3498db',
							eventId = $(this).find('event_id').text(),
                            eventURL = +eventId,
							startTime = $(this).find('event_starttime').text(),
							startSplit = startTime.split(":"),
							endTime = $(this).find('event_endtime').text(),
							endSplit = endTime.split(":"),
							eventLink = '',
                            
                            eventPlace = $(this).find('event_place').text();
						...
						function multidaylist(){
							$('#'+uniqueId+' .monthly-list-item[data-number="'+i+'"]').addClass('item-has-event').append('<a href="'+eventURL+'" class="listed-event"  data-eventid="'+ eventId +'" style="background:'+eventColor+'" title="'+eventTitle+'">'+eventTitle+'<div><div class="monthly-list-time-start">'+fullstartDate+' '+startTime+'</div><div class="monthly-list-time-end">'+fullendDate+' '+endTime+' '+eventPlace+'</div></div></a>');
						}
						...
						if (!fullendDate && startMonth == setMonth && startYear == setYear) {
							...
							$('#'+uniqueId+' .monthly-list-item[data-number="'+startDay+'"]').addClass('item-has-event').append('<a href="'+eventURL+'" class="listed-event"  data-eventid="'+ eventId +'" style="background:'+eventColor+'" title="'+eventTitle+'">'+eventTitle+'<div><div class="monthly-list-time-start">'+startDay+' '+startDate+startTime+'</div><div class="monthly-list-time-end">'+endTime+'</div></div></a>');
		...
		function viewToggleButton(){
			if($('#'+uniqueId+' .monthly-event-list').is(":visible")) {
				...
				$('#'+uniqueId+' .monthly-header-title').prepend('<a href="#" class="monthly-cal" title="Zpátky k prohlídce měsíce"><div></div></a>');
			}
		}
